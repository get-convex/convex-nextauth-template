...
export async function runBasicTests(options: TestOptions) {
  ...
  test("getUser", async () => {
    /// TODO: This is not a good test when IDs are guaranteed
    ///       to come from the DB
    // expect(await adapter.getUser(id())).toBeNull();
    // TODO: Changed to toMatchObject because our adapter adds extra fields
  ...
  maybeTest("listAuthenticatorsByUserId", async () => {
    ...
    /// TODO: Bad test if the ID is guaranteed to come from the DB
    // Test
    ...
    expect(authenticators1 || []).toMatchObject([
      authenticator1,
      authenticator2,
    ]);
    ...
    expect(authenticators2 || []).toMatchObject([authenticator3]);
  });
